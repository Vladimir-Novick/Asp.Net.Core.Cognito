...
namespace Asp.Net.Core.Cognito.Users
{
    public class CognitoUserManager : IDisposable
    {
        ...
        public ApiIdentityResult ChangePasswordWithToken(string username, string token, string newPassword)
        {
            ...
            try
            {
                ...
                using (AmazonCognitoIdentityProviderClient userProvider = GetCognitoIdentityProvider())
                {
                    ...
                    _pool.ConfirmForgotPassword(username, token, newPassword, cancellationToken).ConfigureAwait(false).GetAwaiter().GetResult();
            ...
            using (AmazonCognitoIdentityProviderClient cognito = GetCognitoIdentityProvider())
            {
                try
                {
                    ...
                    if (response)
                    {
                        ...
                        var ret = cognito.AdminConfirmSignUpAsync(signUpConfirmRequest, CancellationToken.None).GetAwaiter().GetResult();
            ...
            using (AmazonCognitoIdentityProviderClient userProvider = GetCognitoIdentityProvider())
            {
                ...
                try
                {
                    ...
                    if (authResponse != null)
                    {
                        ...
                        user2.SessionTokens = new CognitoUserSession(idToken, accessToken, refreshToken, DateTime.Now, DateTime.Now.AddDays(10));
        ...
        private void AddUserTokensToAuthenticationProperties(CognitoUser user, AuthenticationProperties authenticationProperties)
        {
        ...
        public List<ApiUserInfo> GetUserList()
        {
            ...
            using (var provider = (new CognitoUserManager()).GetCognitoIdentityProvider())
            {
                ...
                do
                {
                    ...
                    try
                    {
                        response = provider.ListUsersAsync(request, System.Threading.CancellationToken.None).ConfigureAwait(false).GetAwaiter().GetResult();
